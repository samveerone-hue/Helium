...
void main() {
    ...
    /*
     * Entity yaw rotation:
     * vanilla applies Ry(180-bodyYaw) via Axis.YP.rotationDegrees, which is the
     * standard right-hand-rule rotation about +Y:
     *   x' = x*cos(theta) + z*sin(theta)
     *   z' = z*cos(theta) - x*sin(theta)
     * (Verified directly: rotating +X by +90 degrees about +Y must land on -Z
     * under this convention -- x*c+z*s / z*c-x*s is the only one of the two
     * candidate sign patterns that produces that result.) The previous formula
     * here (x*c-z*s, x*s+z*c) computes R(-theta), the inverse of vanilla's
     * rotation. The two only agree when sin(theta) == 0, i.e. an entity facing
     * exactly north/south, which is why this reads as "sometimes" wrong rather
     * than a constant, easy-to-spot 180 flip -- it's mirrored around the
     * north-south axis for every other yaw.
     */
