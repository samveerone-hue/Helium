...
public class HeliumClient implements ClientModInitializer {
    ...
    private static final ConcurrentHashMap<String, Boolean> featureAvailability = new ConcurrentHashMap<>();
    ...
    public void onInitializeClient() {
        ...
        if (isAndroid && experimental.glStateCache) {
            LOGGER.warn("android detected - disabling experimental gl state cache for compatibility");
            experimental.glStateCache = false;
            experimental.save();
        }
        ...
        initFeatureSafely("GLStateCache", () -> {
            if (experimental.glStateCache) {
                glStateCacheFailed = true;
                featureAvailability.put("GLStateCache", false);
                LOGGER.warn("experimental GL state cache is disabled until renderer-owned invalidation is available");
            }
        }, () -> glStateCacheFailed = true);
    ...
    private void initFeatureSafely(String name, Runnable init, Runnable onFailure) {
        featureAvailability.put(name, true);
        try {
            init.run();
        } catch (Throwable t) {
            featureAvailability.put(name, false);
            LOGGER.error("{} failed to initialize, feature disabled", name, t);
            if (onFailure != null) onFailure.run();
        }
    }

    private static boolean featureAvailable(String name) {
        return featureAvailability.getOrDefault(name, true);
    }
    ...
    public static void setRenderPipelining(boolean enabled) {
        ...
        if (enabled) {
            try {
                ...
                featureAvailability.put("RenderPipeline", true);
            } catch (Throwable t) {
                renderPipelineFailed = true;
                featureAvailability.put("RenderPipeline", false);
    ...
    public static boolean isFastMathAvailable() { return featureAvailable("FastMath"); }
    ...
    public static boolean isAdaptiveSyncAvailable() { return featureAvailable("AdaptiveSync"); }
    public static boolean isTemporalReprojectionAvailable() { return featureAvailable("TemporalReprojection"); }
