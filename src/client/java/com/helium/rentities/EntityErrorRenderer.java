...
public class EntityErrorRenderer {
    ...
    private void buildShader() {
        try {
            ...
            if (vertSrc == null || fragSrc == null) {
                HeliumClient.LOGGER.error("[ErrorRenderer] Could not load error shaders");
                return;
            }
            ...
            if (glGetProgrami(errorShaderProgram, GL_LINK_STATUS) == GL_FALSE) {
                HeliumClient.LOGGER.error("[ErrorRenderer] Shader link failed: {}",
            ...
            HeliumClient.LOGGER.error("[ErrorRenderer] Shader build exception: {}", e.getMessage());
        }
    }
    ...
    private static int compileShader(int type, String src) {
        ...
        if (glGetShaderi(id, GL_COMPILE_STATUS) == GL_FALSE)
            HeliumClient.LOGGER.error("[ErrorRenderer] Shader compile error: {}", glGetShaderInfoLog(id));
        return id;
    }
