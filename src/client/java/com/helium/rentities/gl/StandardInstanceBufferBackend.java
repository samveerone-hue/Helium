...
public final class StandardInstanceBufferBackend implements InstanceBufferBackend {
    ...
    public void upload(int slot, long bytes) {
        if (id == 0 || stagingAddr == 0L) throw new IllegalStateException("instance buffer is deleted");
        if (bytes <= 0) return;
        if (slot < 0 || slot >= slots) throw new IndexOutOfBoundsException("slot=" + slot + ", slots=" + slots);
        if (bytes > slotSize) throw new IllegalArgumentException("upload exceeds slot size: " + bytes + " > " + slotSize);
        org.lwjgl.opengl.GL45C.nglNamedBufferSubData(id, offsetOf(slot), bytes, addrOf(slot));
    }

    public void delete() {
        if (stagingAddr != 0L) {
            MemoryUtil.nmemFree(stagingAddr);
            stagingAddr = 0L;
        }
        if (id != 0) {
            glDeleteBuffers(id);
