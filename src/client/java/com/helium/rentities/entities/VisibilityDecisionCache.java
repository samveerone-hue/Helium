...
/**
 * Optional batching prefilter. The decision is intentionally synchronous because it is only a
 * squared-distance comparison on coordinates already available on the render thread. Results are
 * cached and reused between configured refreshes; no worker thread or future is involved.
 *
 * <p>Callers must invoke this on the render thread because the cache is not synchronized and the
 * decision reads live entity coordinates.</p>
 */
public final class VisibilityDecisionCache {
