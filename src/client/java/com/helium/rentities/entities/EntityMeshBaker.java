...
public class EntityMeshBaker {
    ...
    public synchronized void bake() {
        ...
        try {
            ...
            HeliumClient.LOGGER.error("Entity mesh bake failed; it can be retried", t);
        }
    }
    ...
    private float[] extractFromLivingRenderer(LivingEntityRenderer renderer,
        ...
        if (isDebugLogging()) {
            if (captured.length > 0) {
                ...
                HeliumClient.LOGGER.warn("Extraction finished: captured 0 vertices!");
            }
        }
    ...
    public synchronized MeshStatus ensureMeshFor(EntityType<?> type) {
        ...
        try {
            ...
            meshStatus.remove(type);
            recordExtractionFailure(type, System.nanoTime());
            HeliumClient.LOGGER.error("Dynamic mesh extraction failed for {}", type, t);
            return MeshStatus.UNKNOWN;
        } finally {
            currentBakingTypeIdx = -1;
        }
    ...
    private void saveToCacheInternal(java.io.File targetFile, Map<EntityType<?>, CpuMesh> meshes, float[] pivotDataSnapshot, boolean[] pivotWrittenSnapshot) {
        ...
        try (java.io.DataOutputStream out = new java.io.DataOutputStream(
            ...
            HeliumClient.LOGGER.error("[EntityCache] Failed to serialize cache: {}", e.getMessage());
            return;
        }
        ...
        try {
            ...
            HeliumClient.LOGGER.info("[EntityCache] Saved {} entity types to {}", meshes.size(), f.getPath());
        } catch (Exception e) {
            HeliumClient.LOGGER.error("[EntityCache] Failed to save cache: {}", e.getMessage());
        }
    }
    ...
    public boolean loadFromCache() {
        ...
        try {
            ...
            if (size <= 0 || size > MAX_CACHE_BYTES) {
                HeliumClient.LOGGER.warn("[EntityCache] Invalid cache size {}; ignoring cache", size);
                return false;
            }
        } catch (Exception e) {
            HeliumClient.LOGGER.warn("[EntityCache] Could not inspect cache size: {}", e.getMessage());
            return false;
        }
        ...
        try (java.io.DataInputStream in = new java.io.DataInputStream(
            ...
            if (in.readInt() != CACHE_MAGIC) { HeliumClient.LOGGER.warn("[EntityCache] Bad magic"); return false; }
            if (in.readInt() != CACHE_VERSION) { HeliumClient.LOGGER.warn("[EntityCache] Version mismatch; rebuilding cache"); return false; }

            int typeCount = in.readInt();
            if (typeCount < 0 || typeCount > MAX_ENTITY_TYPES) {
                HeliumClient.LOGGER.warn("[EntityCache] Invalid entity type count {}", typeCount);
                return false;
            }
            ...
            uploadPivotSSBO();
            bakeState = BakeState.READY;
            HeliumClient.LOGGER.info("[EntityCache] Loaded {} entity types from cache", meshInfoMap.size());
            return true;

        } catch (Exception e) {
            HeliumClient.LOGGER.error("[EntityCache] Failed to load cache; ignoring stale/corrupt cache: {}", e.getMessage());
            cpuMeshes.clear();
            meshInfoMap.clear();
            meshStatus.clear();
            bakeState = BakeState.FAILED;
