...
public final class AsyncPackReloader {
    ...
    public static void reloadasync() {
        ...
        if (client == null) { _loading.set(false); return; }
        try {
            HeliumClient.LOGGER.info("async pack reload started");
            if (DeduplicationManager.isenabled()) DeduplicationManager.clearcaches();
            ...
            if (reason != null) client.reloadStateTracker.startReload(reason, packs);
            ReloadInstance reload = client.resourceManager.createReload(
                    RELOAD_EXECUTOR, client, Minecraft.RESOURCE_RELOAD_INITIAL_TASK, packs);
            reload.done().thenRun(() -> {
                _needsrerender = true;
                try { client.reloadStateTracker.finishReload(); } catch (Throwable ignored) {}
                try { client.downloadedPackSource.onReloadSuccess(); } catch (Throwable ignored) {}
    ...
    public static void tick() {
        ...
        if (client != null && client.levelRenderer != null) {
            try { client.levelRenderer.invalidateCompiledGeometry(); } catch (Throwable ignored) {}
        }
        _loading.set(false);
    }
